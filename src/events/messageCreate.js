...
client.on('messageCreate', async message => {
    ...
    if (message.content == (`<@!${client.user.id}>`) || message.content == (`<@${client.user.id}>`)) {
        ...
        message.reply(`Meu prefixo é \`${prefix}\`, para mais detalhes digite \`${prefix}ajuda\`.`);
    ...
    if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
            ...
            message.reply(`Aguarde **${Math.floor(timeLeft)} segundos** antes de executar outro comando!`);
    ...
    if (command.memberPermissions != '') {
        ...
        if (!client.config.bot.ownerId.includes(message.author.id)) {
            ...
            if (neededPermissions.length > 0) {
                ...
                return message.reply(`Sem permissão`).catch(O_o => { })
            };
        };
    };
    ...
    if (command.botPermissions != '') {
        ...
        if (neededPermissions.length > 0) {
            ...
            return message.reply(`Não possuo permissão para executar este comando!\n - Permissões faltando: **${permission(neededPermissions)}**`).catch(O_o => { });
        };
    };

    //! VERIFICAR SE O COMANDO PRECISA DE ARGUMENTOS
    if (command.args && !args.length) {
        return message.reply('Falta argumento').catch(O_o => { });
    };
